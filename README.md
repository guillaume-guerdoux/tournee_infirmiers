...
Based on data such as the heals that have to be performed, their duration, their constraints, the availability times of every nurse and the distance matrix between each patient location, we use a genetic algorithm to generate a planning which is:

* realistic (for instance, no nurse can be in two locations at the same time)
* compatible with both schedules of nurses and patients
* minimizes the distance between each task.

Finding the optimal solution is a very complex problem and requires unreasonable amounts of time and computing resources. Our algorithm allows us to find a "good enough" solution by iterating a genetic process. What we mean by "schedule" is a list of tasks for each nurse in a day.
...
Candidate 1 is considered as better than candidate 2 mean *fitness function(candidate1)* <= *fitness function(candidate2)*
...
The actions that can be performed into a population include:
